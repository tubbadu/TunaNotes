    ...
    : QSyntaxHighlighter(parent)
{
   ...
   /*for (const QString &pattern : keywordPatterns) {
	...
	rule.pattern = QRegularExpression(QStringLiteral("(\\*|\\_)(?!\\s)(.+?)(?<!\\s)(\\*|\\_)"));
    rule.format = italicFormat;
    highlightingRules.append(rule);

	boldFormat.setFontWeight(QFont::Bold);
	rule.pattern = QRegularExpression(QStringLiteral("(\\*|\\_){2}(?!\\s)(.+?)(?<!\\s)(\\*|\\_){2}"));
    rule.format = boldFormat;
    highlightingRules.append(rule);

	bolditalicFormat.setFontWeight(QFont::Bold);
	bolditalicFormat.setFontItalic(true);
	rule.pattern = QRegularExpression(QStringLiteral("(\\*|\\_){3}(?!\\s)(.+?)(?<!\\s)(\\*|\\_){3}"));
    rule.format = bolditalicFormat;
    highlightingRules.append(rule);

	
	monospacedFormat.setFontFamilies(QStringList("Noto Sans Mono")); // todo get font from kirigami
	monospacedFormat.setForeground(Qt::gray); // todo get color from kirigami
	rule.pattern = QRegularExpression(QStringLiteral("`(?!\\s)(.+?)(?<!\\s)`"));
    rule.format = monospacedFormat;
    highlightingRules.append(rule);
